...
public class DespesaRelatorioService {
    ...
    public void exportarPdf(HttpServletResponse response, ColaboradorEntity usuarioLogado, java.util.List<Long> idsdespesas)
            throws DocumentException, IOException {

        log.debug("Método responsável por encaminhar para métodos de construção de PDF de relatório de despesas e " +
        ...
        java.util.List<DespesaEntity> despesas = idsdespesas.isEmpty()
                ? despesaRepositoryImpl.implementaBuscaPorTodos(usuarioLogado.getEmpresa().getId())
                : despesaRepositoryImpl.implementaBuscaPorIdEmMassa(idsdespesas);
        ...
        try (Document document = new Document(PageSize.A4)) {
            ...
            document.add(constroiTabelaInformativos(despesas));
            ...
            acaoService.salvaHistoricoColaborador(usuarioLogado, null,
                    ModulosEnum.DESPESAS, TipoAcaoEnum.RELATORIO, null);
    ...
    private void escreveDadosTabela(PdfPTable table, java.util.List<DespesaEntity> despesas) {
        ...
        log.debug("Iniciando iteração pelos despesas que devem ser exibidos no relatório...");
        for (DespesaEntity despesa : despesas) {

            log.debug("Despesa de id {} em iteração", despesa.getId());
            ...
            cell.setPhrase(new Phrase(ConversorDeDados.converteValorDoubleParaValorMonetario(despesa.getValor()), font));
            table.addCell(cell);
            ...
            String dataAgendamento = !Objects.equals(despesa.getDataAgendamento(), "Pago")
                    ? converteDataUsParaDataBr(despesa.getDataAgendamento())
                    : "Pago";
            cell.setPhrase(new Phrase(dataAgendamento, font));
            table.addCell(cell);
    ...
    public Paragraph constroiParagrafoTitulo() {
        ...
        Paragraph p = new Paragraph("Relatório de despesas", font);
    ...
    public PdfPTable constroiTabelaInformativos(java.util.List<DespesaEntity> despesas) {
        ...
        cell.setPhrase(new Phrase("Total: " + despesas.size(), font));
        table.addCell(cell);
